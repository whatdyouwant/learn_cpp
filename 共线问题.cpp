...
int fun(std::vector<point>& data) {
    ...
    for (int i = 0; i < data.size()-1; i++) {
        for (int j = i+1; j < data.size(); j++) {
            kAndB temp;
            temp.k = static_cast<float>(data[j].y - data[i].y) / static_cast<float>(data[j].x - data[i].x);
            temp.b = static_cast<float>(data[j].y) - temp.k * static_cast<float>(data[j].x);
            map_kb_nums[temp.k][temp.b].insert(to_string(data[j].x) +"."+ to_string(data[j].y));
        }
    }
    int maxn = 0;
...
int main() {
    ...
    std::vector<point> data{{-1, 1},  {0, 0}, {1, 1},
                            {2, 2}, {3, 3}, {3, 4},{6,6}};
